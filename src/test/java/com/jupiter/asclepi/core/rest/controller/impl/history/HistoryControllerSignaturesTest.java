...
class HistoryControllerSignaturesTest extends AbstractHistoryTest {

    private static final FieldDescriptor[] INFO_DESCRIPTORS = new FieldDescriptor[]{
            fieldWithPath("clientId").description("ID of the client to which disease history belongs.").type(JsonFieldType.NUMBER),
            fieldWithPath("number").description("Number of the disease history.").type(JsonFieldType.STRING),
            fieldWithPath("diagnosisId").description("ID of diagnosis for this disease history.").type(JsonFieldType.STRING),
            fieldWithPath("doctorId").description("Middle name of the client in the system").type(JsonFieldType.STRING).optional(),
    };
    ...
    @Autowired
    public HistoryControllerSignaturesTest(ObjectMapper objectMapper, EntityManager entityManager) {
    ...
    void testSuccessfulClientCreationRequestResponseSignatures() throws Exception {
        this.mockMvc.perform(generateCreateRequest(generateCreateParams(false)))
                ...
                .andDo(document("clientSuccessfulCreation",
                        ...
                        responseFields(INFO_DESCRIPTORS)
                ));
    }
        ...
        this.mockMvc.perform(generateGetAllRequest())
                ...
                .andDo(document("clientSuccessfulGettingAll",
                        responseFields(fieldWithPath("[]").description("Array of ClientInfo").type(JsonFieldType.ARRAY))
                                .andWithPrefix("[]", INFO_DESCRIPTORS)
                ));
    }
